...
begin
  ...
  payload := jsonb_build_object(
    ...
    'email_subject',     replace(replace(coalesce(d.outreach_subject, ''), '—', ','), '–', ','),
    'email_body',        replace(replace(coalesce(d.outreach_body, ''), '—', ','), '–', ',') ||
                         case when signature is null or signature = ''
                              then ''
                              else e'\n\n' || signature end,
    'shoot_window_text', replace(replace(coalesce(d.best_shoot_window, ''), '—', ','), '–', ','),
    ...
    'time_rationale',    replace(replace(coalesce(d.time_rationale, ''), '—', ','), '–', ','),
    'shot_list',         replace(replace(shots, '—', ','), '–', ','),
    ...
    'calendar_description',
      ...
      e'\nWindow: '      || replace(replace(coalesce(d.best_shoot_window, ''), '—', ','), '–', ',') ||
      case when coalesce(d.time_rationale, '') = '' then ''
           else e'\nWhy this window: ' ||
                replace(replace(d.time_rationale, '—', ','), '–', ',') end ||
